export default function FeaturesSection() {
  ...
  return (
    <section className="bg-[#faf7ff] py-16 lg:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Top Stats */}
        <div className="grid grid-cols-2 gap-4 rounded-3xl border border-violet-100 bg-white p-6 shadow-sm lg:grid-cols-4 lg:gap-6">
          {topStats.map((item, index) => (
            <div
              key={index}
              className="flex items-center justify-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-md"
            >
              <div
                className={`flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br ${item.color} text-lg text-white shadow-md`}
              >
                {item.icon}
              </div>

              <div>
                <h3 className="text-xl font-bold text-violet-700 lg:text-2xl">
                  {item.value}
                </h3>
